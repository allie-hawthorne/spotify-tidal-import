...
export class SpotifyImporter {
  ...
  getTracksFromPlaylists = async (spotifyPlaylists: Playlist[], selectedPlaylists: Playlist[]): Promise<PlaylistWithTracks[]> => {
    ...
    const updatedPlaylists: PlaylistWithTracks[] = selectedSpotifyPlaylists.map((_, i) => {
